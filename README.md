...
**~730 KB static binary (macOS arm64), ~5.7 MB (Linux x86_64, statically linked with OpenSSL + sqlite3). 6 LLM providers. 9 built-in tools. Telegram channel (+ WhatsApp opt-in). Persistent memory with knowledge graph and vector search. Compile-time feature flags to strip what you don't need.**
...
| Static (`make build-static`, stripped) | ~730 KB | ~5.7 MB |
