...
ChatResponse OpenRouterProvider::chat(const std::vector<ChatMessage>& messages,
    ...
    // Build messages array (same as OpenAI format)
    ...
    // Build tools array (same as OpenAI format)
    if (!tools.empty()) {
    ...
    // Make HTTP request
    std::string url = std::string(BASE_URL) + "/chat/completions";
    std::vector<Header> headers = {
        {"Authorization", "Bearer " + api_key_},
        {"Content-Type", "application/json"},
        {"HTTP-Referer", "https://ptrclaw.dev"},
        {"X-Title", "PtrClaw"}
    };
    ...
    // Parse choices (same as OpenAI format)
...
ChatResponse OpenRouterProvider::chat_stream(const std::vector<ChatMessage>& messages,
                                               const std::vector<ToolSpec>& tools,
                                               const std::string& model,
                                               double temperature,
                                               const TextDeltaCallback& on_delta) {
    // Build request (same as chat() request building)
    json request;
    request["model"] = model;
    request["temperature"] = temperature;
    request["stream"] = true;
...
std::string OpenRouterProvider::chat_simple(const std::string& system_prompt,
