...
class CmdMockProvider : public Provider {
public:
...
static Agent make_cmd_agent() {
    auto provider = std::make_unique<CmdMockProvider>();
...
// RAII guard that redirects HOME to a temp dir with optional skills
struct HomeGuard {
    std::string old_home;
    std::filesystem::path tmpdir;
    std::filesystem::path skills_dir;

    explicit HomeGuard(bool create_skills_dir = true)
        : tmpdir(std::filesystem::temp_directory_path() /
                 ("ptrclaw_cmd_home_" + std::to_string(getpid()) + "_" +
...
TEST_CASE("cmd_status: returns provider and model info", "[commands]") {
    ...
    REQUIRE(result.find("Provider: mock") != std::string::npos);
...
TEST_CASE("cmd_skill: activate shows active tag and tools", "[commands]") {
    ...
    auto agent = make_cmd_agent();
    agent.load_skills(); // load from redirected HOME
